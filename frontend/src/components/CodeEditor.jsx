...
export default function CodeEditor({ 
  ...
  // Support Tab Key in standard Textarea
    ...
    try {
      ...
      if (data.error) {
        ...
        if (!data.stdout && !data.stderr) {
          output = "Program executed successfully with no console output.";
        }
        setConsoleOutput(output);
      }
    } catch (error) {
      setConsoleOutput(`Error compiling code: ${error.message}`);
    } finally {
      setIsRunning(false);
    }
  };
    ...
    <div className="editor-container">
      <div className="editor-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          {isLanguageEditable ? (
            ...
            <span style={{ fontWeight: '600', color: 'var(--accent-light)' }}>{language.toUpperCase()}</span>
          )}
        </div>
        <button 
      ...
      <div className="console-panel">
        <div className="console-stdin">
          <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)', fontWeight: '600' }}>STDIN:</span>
          <input 
            type="text" 
            placeholder="Type input inputs here for testing (optional)..." 
            value={stdin} 
        ...
        <div className="console-body" style={{ whiteSpace: 'pre-wrap', color: consoleOutput.startsWith('Error') ? 'var(--error)' : 'var(--text-main)' }}>
          {consoleOutput || "Run your program to see outputs..."}
        </div>
      </div>
    </div>
  );
